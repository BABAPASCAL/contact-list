...
class ContactsExploreApp(tk.Tk):
    ...
    def add_contact(self):
        ...
        save_button = tk.Button(add_contact_window, text="Save", command=lambda: self.save_contact(name_entry.get(), phone_entry.get(), add_contact_window), bg="black", fg="white")
        save_button.pack(pady=5)
    ...
    def search_contact(self):
        ...
        search_button = tk.Button(search_contact_window, text="Search", command=lambda: self.display_search_result(search_entry.get()), bg="black", fg="white")
        search_button.pack(pady=5)
    ...
    def display_search_result(self, search_name):
        if search_name in self.contacts:
            ...
            result_label = tk.Label(result_window, text=f"{search_name}'s contact number is {self.contacts[search_name]}", bg="black", fg="white")
            result_label.pack(pady=20)
        else:
            not_found_window = tk.Toplevel(self)
            not_found_window.title("Search Result")
            not_found_window.geometry("200x100")
            not_found_window.configure(bg="black")
    ...
    def display_contacts(self):
        ...
        display_label = tk.Label(display_window, text="Name\t\tContact Number", bg="black", fg="white")
        display_label.pack(pady=10)

        for name, phone in self.contacts.items():
            contact_info_label = tk.Label(display_window, text=f"{name}\t\t{phone}", bg="black", fg="white")
            contact_info_label.pack()

    def edit_contact(self):
        edit_contact_window = tk.Toplevel(self)
        edit_contact_window.title("Edit Contact")
        edit_contact_window.geometry("300x150")
        edit_contact_window.configure(bg="black")

        edit_name_label = tk.Label(edit_contact_window, text="Name:", bg="black", fg="white")
        edit_name_label.pack(pady=5)
        edit_name_entry = tk.Entry(edit_contact_window)
        edit_name_entry.pack(pady=5)

        edit_phone_label = tk.Label(edit_contact_window, text="Phone:", bg="black", fg="white")
        edit_phone_label.pack(pady=5)
        edit_phone_entry = tk.Entry(edit_contact_window)
        edit_phone_entry.pack(pady=5)

        update_button = tk.Button(edit_contact_window, text="Update", command=lambda: self.update_contact(edit_name_entry.get(), edit_phone_entry.get(), edit_contact_window), bg="black", fg="white")
        update_button.pack(pady=5)

    def update_contact(self, name, phone, window):
        if name in self.contacts:
            self.contacts[name] = phone
            window.destroy()
            self.display_contacts()
